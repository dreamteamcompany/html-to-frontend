...
export const exportSavingsToExcel = (items: SavingsItem[], periodLabel: string): void => {
  const HEADERS = ['Дата', 'Отдел-заказчик', 'Сервис', 'Описание / Комментарий', 'Сумма (₽)'];
  ...
  const dataRows = items.map((item, i) => {
    const rowNum = i + 2;
    return `<row r="${rowNum}">
      ${cell(item.created_at ? fmtDate(item.created_at) : '—')}
      ${cell(item.department_name || '—')}
      ${cell(item.service_name || '—')}
      ${cell(item.description || '—')}
      ${cell(fmtAmount(item.amount))}
    </row>`;
  });

  const totalRow = `<row r="${items.length + 2}">
    ${cell('ИТОГО')}${cell('')}${cell('')}${cell('')}
    ${cell(fmtAmount(items.reduce((s, p) => s + p.amount, 0)))}
  </row>`;

  const sheetData = [headerRow, ...dataRows, totalRow].join('\n');
  const colWidths = [16, 28, 22, 40, 16]
    .map((w, i) => `<col min="${i + 1}" max="${i + 1}" width="${w}" customWidth="1"/>`)
    .join('');
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF01B574"/></patternFill></fill></fills>
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets><sheet name="Экономия" sheetId="1" r:id="rId1"/></sheets>
</workbook>`;
