...
const getStatusBadge = (status?: string) => {
  if (!status || status === 'draft') {
    return <span className="px-2 py-1 rounded-full text-xs bg-gray-500/20" style={{ color: '#1e3a8a' }}>Черновик</span>;
  }
  if (status === 'pending_ceo') {
    return <span className="px-2 py-1 rounded-full text-xs bg-blue-500/20" style={{ color: '#1e3a8a' }}>Ожидает CEO</span>;
  }
  if (status === 'approved') {
    return <span className="px-2 py-1 rounded-full text-xs bg-green-500/20" style={{ color: '#1e3a8a' }}>Одобрен</span>;
  }
  if (status === 'rejected') {
    return <span className="px-2 py-1 rounded-full text-xs bg-red-500/20" style={{ color: '#1e3a8a' }}>Отклонен</span>;
  }
  if (status === 'revoked') {
    return <span className="px-2 py-1 rounded-full text-xs bg-orange-500/20" style={{ color: '#1e3a8a' }}>⚠ Отозван</span>;
  }
  return null;
};
