                ...
                options={{
                  ...
                  scales: {
                    r: {
                      ...
                      min: 0,
                      ticks: { display: false },
                      grid: { color: 'rgba(1, 181, 116, 0.15)', lineWidth: 1 },
                      angleLines: { color: 'rgba(1, 181, 116, 0.2)', lineWidth: 2 },
                      pointLabels: {
                        padding: isMobile ? 10 : 15,
                        font: { size: isMobile ? 11 : 14, weight: '700' },
                        callback: (label: string, index: number) => {
                          const dept = currentData[index];
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {currentData.slice(0, 3).map((dept, index) => (
                  <div key={index} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'rgba(255, 255, 255, 0.03)', borderRadius: '8px', border: '1px solid rgba(1, 181, 116, 0.15)' }}>
                    ...
                    <span style={{ fontSize: isMobile ? '14px' : '16px', color: '#01b574', fontWeight: '800', textShadow: '0 0 10px rgba(1, 181, 116, 0.5)' }}>
                      {new Intl.NumberFormat('ru-RU').format(dept.amount)} ₽
                    </span>
                  </div>
                ))}
              </div>
