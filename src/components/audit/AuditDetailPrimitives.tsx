...
export const FieldDiff = ({ field, oldVal, newVal }: { field: string; oldVal: unknown; newVal: unknown }) => (
  <div className="flex flex-wrap items-center gap-1.5 text-xs py-2"
    ...
    <span className="px-2 py-0.5 rounded bg-red-500/10 text-red-700 dark:text-red-300 line-through break-all">
      {formatValue(oldVal, field)}
    </span>
    <Icon name="ArrowRight" size={11} className="text-muted-foreground/40 flex-shrink-0" />
    <span className="px-2 py-0.5 rounded bg-green-500/10 text-green-700 dark:text-green-300 break-all">
  ...
  <div className="flex items-start gap-3 py-1.5 text-xs"
    ...
    <span className={
      highlight === 'green' ? 'text-green-700 dark:text-green-300' :
        highlight === 'red' ? 'text-red-700 dark:text-red-300 line-through opacity-70' :
          'text-foreground/90'
    }>{value}</span>
  </div>
);
...
export const ObjectInfoCard = ({ data, highlight, title }: { data: Record<string, unknown>; highlight: 'green' | 'red'; title: string }) => {
  ...
  const iconClass = highlight === 'green' ? 'text-green-600 dark:text-green-300' : 'text-red-600 dark:text-red-300';

  return (
    <div className="mb-3">
      <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-2">
        <Icon name={iconName} size={12} className={iconClass} />
        <span>{title}</span>
      </div>
